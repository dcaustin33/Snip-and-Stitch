...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        help="List of generated image paths",
    )
    parser.add_argument(
        "--output_path", type=str, help="Path to save the output images in ranked order"
    )
    ...
    for i, directory in enumerate(args.generated_image_paths):
        ...
        for im_dir in os.listdir(directory):
            ...
            if im_dir.split("/")[-1] in ["ranked", "generations"]:
                continue
    ...
    similarity = CLIP_text_similarity(
        generated_images, model, processor, args.prompt, args.device
    )
